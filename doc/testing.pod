...
test-manager/ - An automatic unit-testing framework for MIT Scheme or (formerly) Guile
...
Also, as of version 1.2, I have abandoned the Guile port.  Most of the
facilities described above should work in Guile as well as MIT Scheme.
The exceptions I know are that C<produces> does not work in the Guile
REPL (though it does still work inside C<interaction>) which rather
defeats its purpose; that C<generic-match> is not actually a generic
procedure in Guile (though that could presumably be fixed by one who
knew Guile's generic procedure facilities); and that C<check> does not
accept a message argument in Guile.
