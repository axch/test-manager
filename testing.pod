...
Define a one-expression anonymous test.  If you have many independent
assertions you need to make and you don't want to invent names for
each individual one, this is the test definition syntax for you.
...
Passes iff the given value is a true value (to wit, not #f).  The
optional C<message> argument is included in the failure report if the
assertion fails.

=item (assert-false thing [message])

Passes iff the given value is a false value (to wit, #f).  The
optional C<message> argument is included in the failure report if the
assertion fails.
...
Passes iff the given regular expression matches the given string.  The
optional C<message> argument is included in the failure report if the
assertion fails.
...
values.  The optional C<message> argument is included in the failure
